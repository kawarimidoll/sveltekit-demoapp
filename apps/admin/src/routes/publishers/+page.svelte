<script lang='ts'>
  ...
  function drawerOpen(publisher: Partial<typeof data.publishers[number]>) {
    checked = true;
  ...
  <div class='drawer-content'>
    ...
    <section class='m-auto w-full'>
      <div class='mb-4 mt-8 flex'>
        <h2 class='my-0 mr-8 block'>Publishers</h2>
        {#if data.isSuper}
      ...
      {#if data.publishers.length === 0}
        <div class='my-8 w-full text-center'>No publishers found!</div>
      {/if}
      <Table headers={[
        ...
        {#snippet tbody()}
          {#each data.publishers as publisher}
            <tr class='hover:bg-gray-100 dark:hover:bg-neutral-700'>
              ...
              <td>
                {format(publisher.updatedAt, 'yyyy-MM-dd')}<br>
                {format(publisher.updatedAt, 'HH:mm:ss')}
              </td>
              {#if data.isSuper}
                <td>
                  <button class='btn btn-circle btn-outline drawer-button btn-sm'
                          aria-label='edit publisher'
                          onclick={() => { drawerOpen(publisher); }}>
